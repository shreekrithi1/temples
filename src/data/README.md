...
The user-authorized additional budget is **USD 20**. `you-settlement-plan.json` contains 4,000 geographically distributed queries drawn from the GeoNames cities500 index (CC BY 4.0). This index includes places above 500 population and administrative seats; it is not an inventory of every village. The queue includes continents, countries/territories, 2,000 Indian settlements, and 1,740 other settlements.

`npm run data:crawl` reads `YOU_API_KEY` from `.env.local`. A persistent `you-settlement-budget.json` reserves each attempted request before sending it. Retries count toward the same 4,000-attempt ceiling at the documented base price of $0.005/call, with no extraction add-ons. The ledger is an upper-bound estimate, not an account invoice. **Do not delete or reset this ledger to resume**; cached successful queries are reused. A process lock prevents concurrent crawlers sharing the budget.
